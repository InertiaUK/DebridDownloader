...
**Goal:** Redesign the Settings page from a flat single-column scroll to a two-column macOS System Settings layout with category nav, grouped setting cards, scroll spy, and collapsible add-tracker form.

**Architecture:** Single file rewrite of `src/pages/SettingsPage.tsx`. All state, handlers, effects, and logic remain unchanged — only the JSX layout and the `ToggleRow` component are restructured. New additions: category nav with icons, setting card grouping, IntersectionObserver scroll spy, collapsible add-tracker form state.

**Tech Stack:** React, TypeScript, Tailwind CSS, IntersectionObserver API.

**Spec:** `docs/superpowers/specs/2026-03-22-settings-redesign-design.md`
...
|------|--------|---------------|
| `src/pages/SettingsPage.tsx` | Modify | Complete JSX rewrite: two-column layout, category nav, setting cards, scroll spy, collapsible form. All existing logic preserved. |

---

### Task 1: Two-Column Layout with Category Nav and Setting Cards

Restructure the JSX from single-column to two-column layout. Group settings into bordered cards. Add left sidebar category nav with icons.

**Files:**
- Modify: `src/pages/SettingsPage.tsx`

- [ ] **Step 1: Rewrite the main layout and category nav**

Replace everything from the `if (loading)` guard (line 176) through the closing brace of `SettingsPage` (line 633). This includes the loading guard, the `accentColor` declaration, and the main return JSX. Keep all code above line 176 (imports, interfaces, state, handlers, effects — lines 1-175) as-is. The `ToggleRow` component below (lines 635-679) is replaced with an updated version.

The new return should be:
...
This requires three new pieces of state and a helper function. Insert these in the component after the existing add-tracker form state (after line 60, before the `useEffect`). No new imports needed — `useState` and `useRef` are already imported.
...
And update the `ToggleRow` component at the bottom to fit within card rows (remove outer margin, use card-row padding):
...
git commit -m "feat: redesign Settings page with two-column layout and setting cards"
```

---

### Task 2: Add Scroll Spy

Wire up IntersectionObserver to highlight the active category in the left nav as the user scrolls.

**Files:**
- Modify: `src/pages/SettingsPage.tsx`

- [ ] **Step 1: Add IntersectionObserver effect**

After the existing `useEffect` blocks (after the settings loading effect), add this scroll spy effect:
...
The `!!settings` dependency ensures the observer is set up once after settings load and sections render, without re-creating on every settings change.
...
Test:
1. Navigate to Settings — should see two-column layout
2. Click categories in left nav — right panel scrolls to section
3. Scroll the right panel — active category in left nav updates
4. All toggles, selects, and buttons function as before
5. Add a tracker using the collapsible form
6. Switch provider — disabled state + reload works

- [ ] **Step 4: Commit**

```bash
git add src/pages/SettingsPage.tsx
git commit -m "feat: add scroll spy to Settings category navigation"
```
